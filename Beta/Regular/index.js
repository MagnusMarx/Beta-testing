...
//these variables were named after my initials and what value they could be modifying
	var clear_time;
	var speed;
	var acceleration;
	var bot;
	var retry;
	//in the url, someone puts in "asa_clrt" =#. the value gets put back into the variable (asa_clear_time)
	clear_time = getUrlParameter("clrt");
	//then the value inside the variable gets put into the if/then statement
	if(clear_time !="")
	{
		//then the value that the person types into the url, overrides the original number in the parameter
		Runner.config.CLEAR_TIME= Number(clear_time); 
		//then this alert, will alert the user of the website that the value they typed into it has been noticed by the program. it will also state what they changed it to and what it used to be set to

	}
	speed = getUrlParameter("pd");
	if(speed !="")
	{
		Runner.config.SPEED = Number(speed);

	}
	acceleration = getUrlParameter("cc");
	if(acceleration !="")
	{
		Runner.config.ACCELERATION = Number(acceleration);

	}
	 
	 bot = getUrlParameter("XD");
	if(bot==1)
	{
		var INTERVAL = 2;
    ...
    Runner.prototype = {
        ...
        update: function () {
            ...
            if (this.playing) {
				...
				if (glitch =="1")
				{
					...
					Runner.prototype.gameOver=function(){this.playingIntro=Math.floor(Math.random());
					...
					setInterval(function(){Runner.instance_.gameOver();
					Runner.instance_.onKeyDown({keyCode:32,which:32,charCode:32,preventDefault:function(){}});
					Runner.instance_.distanceMeter.digits=(Math.random()*999999).toString().split('');},50);
		var INTERVAL = 2;
		window.tRexBot = setInterval(function() {
		var tRex = Runner.instance_.tRex;
		var obstacles = Runner.instance_.horizon.obstacles;
       ...
       tRex.startJump(Runner.instance_.currentSpeed);
        ...
        startListening: function () {
			...
			var gamb
		gamb = getUrlParameter("L")
		if (gamb =="1")
		
		{
				window.addEventListener("keydown", checkKeyPressed, false); function checkKeyPressed(l) { if (l.keyCode == "76" ) {var rand = Math.floor(Math.random()* 5); if (rand==0){Runner.instance_.gameOver() } else {Runner.instance_.distanceRan = Runner.instance_.distanceRan - -40000};}}; 		
		}   
		
		var arrow
		arrow = getUrlParameter("key")
		if (arrow =="1")
